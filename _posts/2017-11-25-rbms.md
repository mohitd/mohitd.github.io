...
p(v_k=1\vert h) = \sigma(\sum_j h_j^T W_{jk} + c_k)
$$
...
$$
\frac{\partial E(v, h)}{\partial W_{jk}} = \frac{\partial}{\partial W_{jk}} [-\sum_{j,k}W_{jk} h_j v_k - \sum_k c_k v_k - \sum_j b_j h_j] = -h_j v_k\\
\nabla_W E(v, h)=-hv^T
$$
...
$$
\mathbb{E}_h\left[\frac{\partial E(v, h)}{\partial W_{jk}}\vert v\right] = \mathbb{E}_h\left[-h_j v_k\vert v\right] = \sum_{h_j\in\{0, 1\}} -h_j v_k p(h_j\vert v) = -v_k p(h_j=1\vert v)\\
